...
def load_maya(db):
    ...
    for id_, name, fc, tc, fixed, percent, rate, p50, p95 in [
        ("eur-local", "Demo SEPA · local", "EUR", "EUR", 0, 0, 1, 0, 0),
        ("vnd-standard", "Demo VND → EUR · standard", "VND", "EUR", 50000, 0.002, 0.000036, 2, 4),
        ("vnd-express", "Demo VND → EUR · express", "VND", "EUR", 150000, 0.005, 0.000036, 1, 1),
        ("usd-standard", "Demo USD → EUR", "USD", "EUR", 3, 0.002, 0.90, 1, 3),
    ]:
        if not db.get(TransferRoute, id_):
            db.add(
                TransferRoute(
                    id=id_,
                    provider_name=name,
                    ...
                    fx_markup=0,
                    min_transfer=0,
                    max_transfer=100000000 if fc == "VND" else 100000,
                    settlement_p50_days=p50,
                    settlement_p95_days=p95,
                )
            )
    review, _ = evidence(
        db,
