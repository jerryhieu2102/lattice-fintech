"use client";
import Link from "next/link";
import { usePathname } from "next/navigation";
import { useState } from "react";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import {
  LayoutDashboard,
  ListChecks,
  Wallet,
  Network,
  FileText,
  Route,
  FlaskConical,
  LifeBuoy,
  ShieldCheck,
  History,
  BarChart3,
  ArrowRight,
  RefreshCw,
  Layers3,
  LogOut,
  CheckCircle2,
  X,
} from "lucide-react";
import { api } from "@/lib/api";
import type { Actor } from "@/lib/types";
import { Overview, FundingPage, CommitmentsPage } from "./finance";
import { DocumentsPage } from "./documents";
import { GraphPage } from "./graph";
import { PlanPage, ScenarioPage, RescuePage } from "./planning";
import { PermissionsPage, AuditPage, BenchmarkPage } from "./governance";
import { Loading, Notice } from "./ui";
const pages = [
  ["", "Overview", LayoutDashboard],
  ["commitments", "Commitments", ListChecks],
  ["funding", "Funding", Wallet],
  ["graph", "LATTICE Graph", Network],
  ["documents", "Documents", FileText],
  ["plan", "Plan", Route],
  ["scenarios", "Scenario Lab", FlaskConical],
  ["rescue", "Rescue Center", LifeBuoy],
  ["permissions", "Permissions", ShieldCheck],
  ["audit", "Audit", History],
  ["benchmark", "Benchmark", BarChart3],
] as const;
export function Workspace() {
  const path = usePathname().split("/")[1] || "";
  const client = useQueryClient();
  const health = useQuery({
    queryKey: ["health"],
    queryFn: () => api<{ as_of: string }>("/health"),
  });
  const auth = useQuery({
    queryKey: ["me"],
    queryFn: () => api<Actor>("/auth/me"),
  });
  const [toast, setToast] = useState<{ text: string; error: boolean } | null>(
    null,
  );
  const [busy, setBusy] = useState(false);
  const notify = (text: string, error = false) => setToast({ text, error });
  async function signIn(role: string) {
    setBusy(true);
    try {
      const actor = await api<Actor>("/auth/login", "POST", {
        email: `${role}@lattice.demo`,
        password: "LatticeDemo2026!",
      });
      client.clear();
      client.setQueryData(["me"], actor);
      notify(`Signed in as ${actor.display_name}`);
    } catch (e) {
      notify((e as Error).message, true);
    } finally {
      setBusy(false);
    }
  }
  async function reset() {
    setBusy(true);
    try {
      await api("/demo/reset", "POST");
      await api("/demo/load-maya", "POST");
      await client.invalidateQueries();
      notify(
        "Maya reset and loaded. The evidence-to-action walkthrough is ready.",
      );
    } catch (e) {
      notify((e as Error).message, true);
    } finally {
      setBusy(false);
    }
  }
  if (auth.isPending) return <Loading />;
  if (!auth.data)
    return (
      <main className="login">
        <div className="login-story">
          <div className="brand">
            <Layers3 /> LATTICE
          </div>
          <span className="eyebrow">
            VERIFY · ORCHESTRATE · STRESS · REPAIR
          </span>
          <h1>
            Every commitment.
            <br />A plan you can
            <br />
            <em>verify.</em>
          </h1>
          <p>
            Cross-border finances are more than a balance.
            <br />
            Connect evidence, funding, deadlines, and the people who make it
            possible.
          </p>
          <div className="login-line">
            <span>VND</span>
            <i />
            <ShieldCheck />
            <i />
            <span>EUR</span>
          </div>
          <small>
            FICTIONAL MAYA DATA · DETERMINISTIC RATES · SANDBOX ONLY
          </small>
        </div>
        <div className="login-card">
          <span className="eyebrow">LATTICE v1.0</span>
          <h2>Open the demo workspace</h2>
          <p>
            Choose an actor to explore the same financial plan through different
            permissions.
          </p>
          {[
            ["student", "Maya Nguyen", "Student workspace"],
            ["parent", "Minh Nguyen", "Parent · own contribution only"],
            [
              "admin",
              "Demo administrator",
              "Reset Maya · simulate institution approval",
            ],
          ].map(([role, name, description]) => (
            <button
              className="account"
              key={role}
              onClick={() => signIn(role)}
              disabled={busy}
            >
              <div>
                <strong>{name}</strong>
                <small>{description}</small>
              </div>
              <ArrowRight size={18} />
            </button>
          ))}
          <Notice>
            New database? Sign in as Demo administrator, then select Reset Maya.
          </Notice>
          <small className="muted">Demo password: LatticeDemo2026!</small>
          {toast && (
            <p role="alert" className="error-text">
              {toast.text}
            </p>
          )}
        </div>
      </main>
    );
  const actor = auth.data;
  const privateAllowed = ["STUDENT", "ADMIN_DEMO"].includes(actor.role);
  const privatePage = ["", "plan", "scenarios", "rescue", "benchmark"].includes(
    path,
  );
  const props = { actor, notify };
  const content =
    privatePage && !privateAllowed ? (
      <Notice danger>
        This is a student-only financial workspace. Your shared commitments and
        own contributions are available from the sidebar.
      </Notice>
    ) : path === "" ? (
      <Overview {...props} />
    ) : path === "funding" ? (
      <FundingPage {...props} />
    ) : path === "commitments" ? (
      <CommitmentsPage {...props} />
    ) : path === "documents" ? (
      <DocumentsPage {...props} />
    ) : path === "graph" ? (
      <GraphPage />
    ) : path === "plan" ? (
      <PlanPage {...props} />
    ) : path === "scenarios" ? (
      <ScenarioPage {...props} />
    ) : path === "rescue" ? (
      <RescuePage {...props} />
    ) : path === "permissions" ? (
      <PermissionsPage {...props} />
    ) : path === "audit" ? (
      <AuditPage />
    ) : path === "benchmark" ? (
      <BenchmarkPage {...props} />
    ) : (
      <Notice>
        Page not found. <Link href="/">Return to overview</Link>.
      </Notice>
    );
  return (
    <div className="app">
      <aside className="sidebar">
        <Link href="/" className="brand">
          <Layers3 size={25} />
          LATTICE<span>v1.0</span>
        </Link>
        <div className="workspace-label">
          <span className="avatar">M</span>
          <div>
            <strong>Maya’s workspace</strong>
            <small>Vietnam → Europe</small>
          </div>
        </div>
        <nav aria-label="Main navigation">
          {pages.map(([href, title, Icon], i) => (
            <Link
              key={href}
              className={`${path === href ? "active" : ""} ${i === 8 ? "nav-separator" : ""}`}
              href={"/" + href}
            >
              <Icon size={18} />
              {title}
              {href === "scenarios" && <span className="dot" />}
            </Link>
          ))}
        </nav>
        <div className="sidebar-bottom">
          <ShieldCheck size={19} />
          <strong>Evidence before action</strong>
          <p>Money moves only in the sandbox.</p>
          <small>DEMO DATA · NOT LIVE RATES</small>
        </div>
      </aside>
      <div className="main">
        <header className="topbar">
          <div>
            <span className="status-dot" />
            DEMO WORKSPACE <span className="top-divider">/</span>
            <span>{pages.find((p) => p[0] === path)?.[1] || "Workspace"}</span>
          </div>
          <div className="top-actions">
            <span className="sandbox">SANDBOX</span>
            <label className="sr-only" htmlFor="actor">
              Demo actor
            </label>
            <select
              id="actor"
              aria-label="Demo actor"
              value={
                actor.role === "ADMIN_DEMO" ? "admin" : actor.role.toLowerCase()
              }
              onChange={(e) => signIn(e.target.value)}
              disabled={busy}
            >
              <option value="student">Maya · Student</option>
              <option value="parent">Minh · Parent</option>
              <option value="sponsor">Sponsor</option>
              <option value="admin">Demo administrator</option>
            </select>
            {actor.role === "ADMIN_DEMO" && (
              <button className="button small" onClick={reset} disabled={busy}>
                <RefreshCw size={14} />
                Reset Maya
              </button>
            )}
            <button
              className="icon-button"
              aria-label="Sign out"
              onClick={async () => {
                await api("/auth/logout", "POST");
                client.clear();
                await client.invalidateQueries();
              }}
            >
              <LogOut size={17} />
            </button>
          </div>
        </header>
        <main className="content">{content}</main>
        <footer>
          Verified cross-border financial orchestration
          <span>
            Planning clock: {health.data?.as_of || "Loading…"} · 45-day horizon
            · No real payments
          </span>
        </footer>
      </div>
      {toast && (
        <div
          role={toast.error ? "alert" : "status"}
          className={`toast ${toast.error ? "error" : ""}`}
        >
          <CheckCircle2 size={18} />
          {toast.text}
          <button
            aria-label="Dismiss notification"
            onClick={() => setToast(null)}
          >
            <X size={16} />
          </button>
        </div>
      )}
    </div>
  );
}
